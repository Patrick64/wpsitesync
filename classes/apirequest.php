...
class SyncApiRequest implements SyncApiHeaders
{
	...
	private $_triggered_push_complete = FALSE;	// set to TRUE if 'spectrom_sync_push_queue_complete' action to be triggered
...
#  SyncDebug::log(__METHOD__.'():' . __LINE__ . ' sending data array: ' . SyncDebug::arr_sanitize($remote_args));
#}
...
SyncDebug::log(__METHOD__ . '():' . __LINE__ . ' api result from "' . $action . '": ' . var_export($request, TRUE));
			...
			} else if (406 === abs($request['response']['code']) && FALSE !== stripos($request['body'], 'This error was generated by Mod_Security.')) {
...
//SyncDebug::log(__METHOD__.'() sync data: ' . var_export($sync_data, TRUE));
		...
		$res = $this->_parse_shortcodes($post_id, $post_data['post_data']['post_content'], $data);
		if (is_wp_error($res))
			return $res;
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' done processing media');
	 ...
	 */
	private function _parse_shortcodes($post_id, $content, &$data)
	{
		return TRUE;
	}
...
SyncDebug::log(__METHOD__.'():' . __LINE__ . ' all images processed; id refs=' . count($this->id_refs) . ' sent images=' . count($this->_sent_images));
		// there are IDs to update, use the 'push_complete' API to indicate completion and update IDs on Target
		if (!$error && (0 !== count($this->id_refs) || 0 !== count($this->_sent_images))) {
...
//SyncDebug::log(__METHOD__."({$code}, '" . var_export($data, TRUE) . "')", TRUE);
		...
		case self::ERROR_UNRESOLVED_PARENT:			$error = __('Content has a Parent Page that has not been Sync\'d. Please Push the Parent page.', 'wpsitesynccontent'); break;
